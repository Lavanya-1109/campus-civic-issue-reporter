...
\usepackage{graphicx}
\usepackage{amssymb} % kept for general math symbols
\usepackage{tikz}
...
\usepackage{xcolor}
\usepackage{caption}
\usepackage{subcaption}   % for placing the two DFDs side by side on one page
\usepackage{float}
\usepackage{xspace}
\usepackage{url}          % for clean bibliography URLs
\usepackage{hyperref}
\usepackage{titlesec}

% ---- Remove the red hyperref boxes but keep everything clickable ----
% "hidelinks" strips the colored text AND the border box that hyperref
% draws by default, while leaving every \ref, \cite and \url fully
% clickable in the PDF viewer.
\hypersetup{hidelinks}

\newcommand{\sysname}{Civic Issue Reporter\xspace}

% ==================== Title formatting: right-aligned ====================
\titleformat{\chapter}[display]
  {\normalfont\huge\bfseries\raggedleft}
  {\chaptertitlename\ \thechapter}{20pt}{\Huge\raggedleft}
\titleformat{\section}
...
\begin{titlepage}
  ...
  \vspace{6pt}
  \texttt{1024030227} Lavanya Sharma\par
  \texttt{1024030309} Parismita Thapa\par
  \texttt{1024030389} Rudra Yadav\par
  \texttt{1024030291} Nadeem Esrar\par

  \vspace{1.2cm}
  \large Group: \texttt{3C22} \quad BE Third Year, Computer Engineering (COE)\par
  \vspace{1.2cm}
...
Figure~\ref{fig:dfd-combined} presents the Gane--Sarson data
flow diagrams for the system. The Context Diagram (Level 0, top) fixes
the system boundary and its external entities -- Student/Faculty,
Administrator, and Super-Admin -- and the major information flows
between them and the system. The Level-1 diagram (bottom) decomposes
the system into its major processes (submission and duplicate check,
routing, status update, and escalation/notification) and the data
stores they read from or write to.

% Both DFDs are now combined into a single figure so they render
% together on one page instead of being split across two pages.
\begin{figure}[H]
  \centering
  \begin{subfigure}[b]{0.62\textwidth}
    \centering
    \includegraphics[width=\textwidth]{dfd_level0.pdf}
  ...
  \begin{subfigure}[b]{0.95\textwidth}
    ...
    \label{fig:dfd1}
  \end{subfigure}
  \caption{Context and Level-1 data flow diagrams for the Civic Issue Reporter system.}
  \label{fig:dfd-combined}
\end{figure}
...
engineering literature~\cite{bettenburg2008duplicate}, so that no
...
% Every entry below carries a real, verifiable URL (publisher page,
% official documentation, or the paper's own hosted PDF), matched
% against the source itself rather than invented.
% ===================================================================
...
\url{https://www.oreilly.com/library/view/uml-distilled-a/0321193687/}

\bibitem{elmasri2015}
R.~Elmasri and S.~B.~Navathe, \emph{Fundamentals of Database Systems},
7th ed. Pearson, 2015.
\url{https://www.pearson.com/en-us/subject-catalog/p/fundamentals-of-database-systems/P200000003546}
...
\bibitem{bettenburg2008duplicate}
N.~Bettenburg, R.~Premraj, T.~Zimmermann, and S.~Kim, ``Duplicate Bug
Reports Considered Harmful \dots Really?,'' in \emph{Proc. 24th IEEE
Int. Conf. Software Maintenance (ICSM)}, Beijing, China, 2008,
pp.~337--345, doi:10.1109/ICSM.2008.4658082.
\url{https://thomas-zimmermann.com/publications/files/bettenburg-icsm-2008.pdf}
