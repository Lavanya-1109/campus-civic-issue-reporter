...
\usepackage{float}
\usepackage{xspace}
\usepackage[hidelinks]{hyperref} % FIX: hidelinks removes the colored/boxed borders on TOC + \ref links
...
\begin{figure}[H]
  \centering
  % NOTE: upload your use case diagram image into the Overleaf file tree
  % (the upload/up-arrow icon above the file list) and make sure the
  % filename below matches it EXACTLY (case-sensitive). Nothing will
  % render here until that file actually exists in the project.
...
\begin{figure}[H]
  \centering
  % NOTE: upload level1_dfd.png into the Overleaf file tree (the
  % upload/up-arrow icon above the file list), exactly like
  % use_case_diagram.png above -- the filename must match exactly.
  \includegraphics[width=0.95\textwidth]{level1_dfd.png}
...
2024. \url{https://www.postgresql.org/docs/}
...
IETF, 2015. \url{https://www.rfc-editor.org/rfc/rfc7519.html}
...
in \emph{Proc. IEEE Int. Conf. Software Maintenance (ICSM)}, 2008, pp.
337--345. \url{https://thomas-zimmermann.com/publications/files/bettenburg-icsm-2008.pdf}
\end{thebibliography}
\end{document}