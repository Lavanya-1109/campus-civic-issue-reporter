...
\author{%
  Lavanya Sharma, COE%
    \thanks{Roll No: \texttt{[1024030227]}, Email: \texttt{lsharma3\_be24@thapar.edu}}
  \and
  Parismita Thapa, COE%
  ...
  \and
  Nadeem Esrar, COE%
    \thanks{Roll No: \texttt{[1024030291]}, Email: \texttt{nesrar\_be24@thapar.edu}}
}
...
\noindent\textbf{Why this matters now:} TIET's campus spans multiple hostels,
...
\textbf{Primary goal:} To design and deploy a role-based web platform
that gives the TIET campus community a structured, trackable channel for
reporting and resolving infrastructure issues, while reducing duplicate
reporting and improving response time to high-priority faults.
...
\noindent Students and faculty are treated identically: both can submit reports,
...
\noindent The comparison set is restricted to issues still open (not
...
surfaces the top candidate to the reporter for a human decision, and it
never fully auto-merges, auto-deletes, or auto-rejects a submission and 
keeping a human in the loop for a step that directly affects data
quality.

\subsection{Web Architecture}

A typical 3-tier web architecture is shown below:
...
\textbf{Frontend} &
React &
Responsive UI for reporters (submission form, status view) and administrators (dashboard), usable on low-to-mid range student devices. \\
\hline
\textbf{Backend API} &
Node.js/Express (or Flask as a fallback) &
REST API for authentication, submission, routing, status transitions, and escalation logic. \\
\hline
\textbf{Database} &
PostgreSQL &
Maintains relational integrity between issues, departments, users, upvotes, and notifications. Supports indexing for duplicate detection and dashboards using fields such as location, department, and status. \\
\hline
\end{tabular}
\caption{Three-Tier Web Architecture}
\label{tab:web-architecture}
\end{table}

\noindent Supporting and Infrastructure: Cloudinary or AWS S3 for image attachments,
...
\begin{itemize}[leftmargin=0.7cm]
  ...
  \item Duplicate-location matching is a structured field comparison
    ...
    geospatial radius calculation which is a deliberate trade-off, since GPS
...
The schedule below breaks delivery into weekly phases, in line with the
iteration cadence described under Time-to-value, and includes an
explicit buffer week to absorb slippage rather than letting it erode the
final wrap-up:
...
\begin{tabularx}{\textwidth}{@{}c l X X@{}}
  \toprule
  \textbf{Wk} & \textbf{Phase} & \textbf{Task} & \textbf{Deliverable} \\
  \midrule
  ...
  12     & \textbf{Buffer} & Bug fixes; unresolved scope & Stabilized build \\
  13--14 & Wrap-up     & Metric analysis (\S\ref{sec:eval-primary}); report prep & Final report , presentation \\
  \bottomrule
\end{tabularx}
\end{center}
...
\textbf{Human resources:} A 4-member team dividing work across
frontend (React UI for reporters and dashboards), backend/API
(authentication, routing, status and escalation logic), duplicate-
detection and data modelling (heuristic design, PostgreSQL schema), and
DevOps (CI/CD, staging deployment, pilot coordination), coordinated
through a shared Git repository.

\vspace{1em}

\noindent\textbf{Material resources:} GitHub for version control and CI (GitHub
Actions); a managed PostgreSQL instance (free/student tier, e.g.,
Supabase or Railway); Cloudinary/AWS S3 free tier for image storage;
and free-tier hosting (e.g., Vercel/Render) for staging and pilot
deployment, consistent with the Engine Availability Heuristic above.

\vspace{1em}

\noindent\textbf{Budget:} No external funding required; all resources are
available through free or managed-tier services and university-
...
This project proposes a \textbf{Civic Issue Reporter}, a role-based web
platform for reporting, tracking, and resolving campus infrastructure
issues at TIET. Students and faculty will be able to report problems such
as electrical, plumbing, cleanliness, or other infrastructure issues,
while department administrators will receive and manage issues related to
their respective departments. A Super-Admin will provide overall
supervision across departments.

\vspace{1em}

\noindent A major focus of the project is \textbf{reducing duplicate reports}.
When a user submits a new issue, the system will compare it with existing
unresolved issues using three factors: location, department/category, and
similarity between issue descriptions. If a likely duplicate is found, the
existing issue will be shown to the user, who can choose to upvote it
instead of creating another report. The system will not automatically
merge or reject reports, keeping the user involved in the final decision.

\vspace{1em}

\noindent The platform will also include a \textbf{priority and escalation
mechanism}. In the initial MVP, an issue will be marked as high priority
when its upvote count crosses a configurable threshold. The system is
designed so that these thresholds can later be adjusted for different
categories based on data collected during the pilot. This allows the
project to move from a simple volume-based mechanism toward a more
appropriate category- and severity-aware escalation system.

\vspace{1em}

\noindent The system will follow a \textbf{three-tier web architecture} consisting
of a React frontend, a REST-based backend using Node.js/Express (with
Flask as a possible fallback), and a PostgreSQL database. Image
attachments will be stored separately using services such as Cloudinary or
AWS S3. Role-based access control will ensure that students, faculty,
administrators, and the Super-Admin can only perform actions permitted for
their roles.

\vspace{1em}

\noindent The project will be evaluated using measurable metrics rather than only
demonstrating that the application works. Important measures include
\textbf{time-to-first-action}, \textbf{duplicate-detection precision},
and \textbf{escalation precision}. A campus pilot will provide real usage
data that can be used to evaluate the system and tune duplicate-detection
and escalation thresholds. The overall goal is therefore not only to build
a working issue-reporting platform, but to create a system that can
gradually improve its routing, duplicate handling, and prioritization
using evidence collected from actual campus use.
\end{document}
